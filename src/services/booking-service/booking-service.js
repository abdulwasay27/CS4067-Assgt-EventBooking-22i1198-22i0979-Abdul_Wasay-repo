...
async function publishBooking(booking_id, email, status) {
  try {
    ...
    throw new Error(
      "Error while publishing event to rabbitmq: " + error.message
    );
  }
}
...
async function getUserBookings(email) {
  try {
    const result = await pool.query("SELECT * FROM booking WHERE email = $1", [
      email,
    ]);
    return result.rows;
  } catch (error) {
...
app.post("/book", async (req, res) => {
  try {
    ...
    const duplicateBooking = await checkUserBooking({
      email: email,
      event_id: event_id,
    });
    if (duplicateBooking) {
      return res
        .status(400)
        .json({ message: "Duplicate booking is not allowed!" });
    } else {
